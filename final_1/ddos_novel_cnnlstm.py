...
#df = pd.read_csv("data_change.csv", parse_dates=["bidirectional_first_seen_ms"])
...
# === 3. Hybrid CNN-LSTM 模型 (根據 Table 11) ===
model = Sequential()
model.add(Conv1D(filters=32, kernel_size=3, activation='relu', 
    input_shape=(X_train.shape[1], X_train.shape[2]), name="conv1"))
model.add(MaxPooling1D(pool_size=2, name="maxpool1"))
model.add(LSTM(32, return_sequences=True, name="lstm"))
model.add(Dropout(0.2, name="dropout"))
model.add(Flatten(name="flatten"))
model.add(Dense(8, activation='relu', name="dense_hidden"))
model.add(Dense(1, activation='sigmoid', name="output"))  # 二分類輸出
...
# 儲存模型（HDF5 格式）
model.save("novel_cnn_lstm_model.h5")

# 儲存標準化器與特徵選擇器
dump(scaler, "scaler.pkl")
dump(k_best, "kbest.pkl")
