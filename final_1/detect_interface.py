...
idle_timeout_setting=5 # 每筆flow幾秒沒封包就輸出 
...
def flow_handler():  # 負責即時處理每筆flow的特徵並進行預測
    while True:
        ...
        y_prob = model.predict(X_input)[0][0] #取出第一個 batch 的第一個輸出值
        print("Predicted probability of attack:", y_prob)
        if y_prob > 0.5:  # 如果預測為攻擊流量
            show_popup("偵測到攻擊流量！")
            os._exit(0)  # 彈窗後立即結束程式
